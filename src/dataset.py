...
class CracksDataset(Dataset):
    def __init__(self, mode: str, transform: 'src.transforms.Compose'):
    ...
    @staticmethod
    def is_crack_exists(masks: Tensor, threshold: float = 0.01) -> Tensor:
        h, w = masks.shape[-2:]
    ...
    def __getitem__(self, index: int) -> Dict[str, Tensor]:
        ...
        cracks = self.is_crack_exists(masks)
    ...
    def _collate_fn(batch: List[Dict[str, Tensor]]) -> Dict[str, Tensor]:
        images, masks, cracks = zip(*((b['images'], b['masks'], b['cracks']) for b in batch))
        return {
